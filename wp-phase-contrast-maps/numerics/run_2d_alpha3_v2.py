#!/usr/bin/env python3
"""
run_2d_alpha3_v2.py — Same 2D (δ₀, φ_α) scan as run_2d_alpha3.py,
rebuilt on the scripts/stroboscopic/ package.

Output schema is a bit-for-bit match of run_2d_alpha3.py. The default
output path is scan_2d_alpha3_v2.h5 so the two can be diffed; pass
--output to override.

Usage:
    python run_2d_alpha3_v2.py              # full grid (1001 × 64, ~minutes)
    python run_2d_alpha3_v2.py --quick      # 51 × 8, fast validation
    python run_2d_alpha3_v2.py --backend jax
"""
...
# Make scripts/ importable so the new package loads without a pip install.
...
# --- nominal parameters (Hasse-matched, matches legacy run_2d_alpha3.py) ---
OMEGA_M = 1.3
T_M = 2 * np.pi / OMEGA_M
DELTA_T = 0.13 * T_M
N_PULSES = 30
...
def run(n_det: int, n_phi: int, output_path: str, backend_name: str):
    ...
    # Pre-build the coupling once (same for every (δ, φ)).
    C = ops.coupling(ETA, NMAX)
    Cdag = C.conj().T

    # Prepare all initial states (depend on φ_α, not δ).
...
def main():
    ...
    p.add_argument('--quick', action='store_true',
                   help='51 × 8 grid for fast validation')
    ...
    p.add_argument('--output', default=None,
                   help='Output HDF5 path (default: scan_2d_alpha3_v2[_quick].h5)')
    args = p.parse_args()

    if args.quick:
        n_det, n_phi = 51, 8
        default_out = 'scan_2d_alpha3_v2_quick.h5'
    else:
        n_det, n_phi = args.n_det, args.n_phi
        default_out = 'scan_2d_alpha3_v2.h5'
