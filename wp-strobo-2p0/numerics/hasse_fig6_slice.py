...
def base_params(N: int, dt_us: float, alpha: float) -> dict:
    return dict(
        alpha=alpha,
        eta=ETA,
...
def main() -> None:
    print("strobo 2.0  —  Hasse-Fig-6 (phi, theta_0) slice")
    ...
    for alpha in ALPHAS:
        for train in TRAINS:
            ...
            print(f"[{tag}]  N={train['N']}  dt={train['dt_us']*1e3:.0f} ns  |alpha|={alpha}")
            sh = run_sheet(train["N"], train["dt_us"], alpha)
            for k, v in sh.items():
                out[f"{tag}_{k}"] = v
