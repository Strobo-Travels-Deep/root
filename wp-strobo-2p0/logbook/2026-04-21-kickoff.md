...
## 2. Physical parameters (confirmed 2026-04-21)
...
| Ω/(2π)        | 0.178 MHz            | Bare carrier Rabi frequency            |
| Δt            | 0.770 µs             | Inter-pulse spacing                    |
| Δt / T_m      | 1.0056 (+0.56 %)     | **Slightly super-stroboscopic**        |
| t_π (carrier) | 2.809 µs             | = π/Ω, the bare carrier π-time         |

Two pulse-train configurations:

| Train | N | δt (ns) | θ_pulse = Ω·δt (rad) | N · θ_pulse (rad) | Total duration       |
|-------|---|---------|----------------------|-------------------|----------------------|
| T1    | 3 | 100     | 0.1118  (6.41°)      | 0.3355  (19.22°)  | 2·Δt + 3·δt = 1.84 µs|
| T2    | 7 | 50      | 0.05592 (3.20°)      | 0.3914  (22.43°)  | 6·Δt + 7·δt = 4.97 µs|

Per-pulse rotation is weak (θ_pulse ≲ 0.11 rad) — pulses are clearly in the
weak-coupling regime, so a Magnus-expansion picture (WP-E reference models
R1/R2) remains applicable as an interpretive aid if useful.
...
- **No separate closing π/2.** The AC train itself IS the analysis π/2.
...
In Hasse 2024 the AC pulse train IS the analysis π/2. Scanning the train's
global phase φ (equivalently: scanning the spin initial-state azimuth by
−φ, by the lab-frame equivalence `U(φ) = R_z(φ) U(0) R_z(φ)†`) gives the
experimental fringe
...
The **coherence contrast** (the "0.76(3)" in Hasse Fig. 2(b)) is the
fringe amplitude:
...
6. **Logbook.** Kickoff (this file) + post-sweep entry
   [2026-04-21-sweep-complete.md](2026-04-21-sweep-complete.md)
   with carrier-dominance observation, ϑ₀ π-periodicity check
   on δ⟨n⟩, T1-vs-T2 linewidth comparison, and cross-reference to
   Hasse Fig. 6.

## 9. Open questions / decisions to confirm

1. **Rabi rate reconciliation.** Documented in the companion memo
   [2026-04-21-rabi-reconciliation.md](2026-04-21-rabi-reconciliation.md).
   Three values are on record: 0.178 MHz (this sweep), 0.300 MHz (repo
   canonical / Hasse Table II AC), 0.446 MHz (original t_π = 1.122 µs
   interpretation). One lab carrier measurement on the T2 train
   distinguishes all three (|C|_vacuum = 0.35 / 0.57 / 0.79, see
   [plots/00_rabi_calibration.png](../plots/00_rabi_calibration.png)).
   Open pending that measurement.
...
execution plan, output format, and open-question list updated to
match the as-executed sweep. See
[2026-04-21-sweep-complete.md](2026-04-21-sweep-complete.md) for
the companion post-sweep record.*
