#!/usr/bin/env python3
"""
run_coh_theta0_det_v5_rabi5x.py — (ϑ₀, δ) scan with Ω boosted by 5×.

Companion to run_coh_theta0_det_v5.py. Identical parameters except
OMEGA_R is multiplied by RABI_SCALE = 5, meaning the per-pulse rotation
Ω_eff·δt grows from π/(2N) to 5·π/(2N), i.e. total train rotation
5·π/2 = 2.5 π instead of the π/2 calibration.

Grid and wall-time same as v5 baseline (≈ 20 s).

Additionally plots the back-action δ⟨n⟩ = ⟨n⟩_final − |α|², which was
saved in v5 h5 but never rendered.

Outputs:
    numerics/coh_theta0_det_v5_rabi5x.h5
    plots/coh_theta0_det_rabi5x_sigma_{x,y,z}_v5.png
    plots/coh_theta0_det_rabi5x_coh_{abs,arg}_v5.png
    plots/coh_theta0_det_rabi5x_delta_n_v5.png
"""
...
OMEGA_EFF_BASELINE = np.pi / (2 * N_PULSES * DELTA_T)
OMEGA_R_BASELINE = OMEGA_EFF_BASELINE / DEBYE_WALLER
...
def main():
    ...
    out_h5 = args.output or os.path.join(SCRIPT_DIR,
                                         'coh_theta0_det_v5_rabi5x.h5')

    print(f"(ϑ₀, δ) coherence scan — RABI ×{RABI_SCALE:g}  "
          f"(backend={args.backend})")
    print(f"  α={ALPHA}  η={ETA}  ω_m={OMEGA_M}  N={N_PULSES}  "
          f"δt={DELTA_T:.3f}")
    print(f"  Ω_baseline={OMEGA_R_BASELINE:.4f}  Ω={OMEGA_R:.4f}  "
          f"Ω/ω_m={OMEGA_R/OMEGA_M:.3f}")
    print(f"  per-pulse rotation Ω_eff·δt = "
          f"{RABI_SCALE * DEBYE_WALLER * OMEGA_R_BASELINE * DELTA_T / np.pi:.2f} π  "
          f"(baseline = 1/(2N) = {1/(2*N_PULSES):.4f} π)")
    print(f"  train budget N·Ω_eff·δt = "
          f"{N_PULSES * RABI_SCALE * DEBYE_WALLER * OMEGA_R_BASELINE * DELTA_T / np.pi:.2f} π  "
          f"(baseline 0.50 π)")
    ...
    with h5py.File(out_h5, 'w') as f:
        ...
        f.attrs['notes'] = (f'(ϑ₀, δ) scan with Ω boosted {RABI_SCALE}× from '
                            'the π/2-calibrated baseline. Intentionally '
                            'over-rotated (train budget = 2.5 π); see loose '
                            'ends in 2026-04-20-coh-theta0-det.md.')
    ...
    heatmap(sigma_y, theta0_grid, det_rel_grid,
            ...
            'coh_theta0_det_rabi5x_sigma_y_v5.png',
    ...
    heatmap(sigma_z, theta0_grid, det_rel_grid,
            ...
            'coh_theta0_det_rabi5x_sigma_z_v5.png',
    ...
    heatmap(coh_abs, theta0_grid, det_rel_grid,
            ...
            'coh_theta0_det_rabi5x_coh_abs_v5.png',
    ...
    heatmap(coh_arg_deg, theta0_grid, det_rel_grid,
            ...
            'coh_theta0_det_rabi5x_coh_arg_v5.png',
            cmap='twilight', ylabel=r'$\arg(C)$ (deg)',
            vlim=180.0, symmetric=True, alpha_map=coh_abs)
    # Back-action δ⟨n⟩ = ⟨n⟩_final − |α|²
    dn_vlim = max(1.0, float(np.abs(delta_n).max()))
    heatmap(delta_n, theta0_grid, det_rel_grid,
            rf'$\delta\langle n\rangle(\vartheta_0, \delta)$   ({title_suffix})',
            'coh_theta0_det_rabi5x_delta_n_v5.png',
            cmap='PRGn', ylabel=r'$\delta\langle n\rangle$',
            vlim=dn_vlim, symmetric=True)
