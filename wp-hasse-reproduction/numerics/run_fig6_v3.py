#!/usr/bin/env python3
"""
run_fig6_v3.py — Hasse Fig 6 with Hasse-matched (N, δt, Ω) regime.

Round 2 parameter matching (engine v0.9.0 D6 fix):
    N         = 30          (Hasse Appendix D)
    δt/T_m    = 0.13        (Hasse: δt = 100 ns, T_m = 770 ns)
    ω_m·δt    ≈ 0.82 rad    (smearing regime, not revival regime)
    Ω         = 0.0902       (chosen so N·Ω_eff·δt = π/2 with Hasse-matched δt)
    Ω/ω_m     ≈ 0.069        (deeper resolved-sideband than Hasse Table II's 0.231,
                              consistent with Hasse's experimentally calibrated
                              effective per-pulse rotation θ = 0.052 rad)

Same 2D (ϑ₀, ϕ_AC) scan as v2. Cost: 64×64 = 4096 runs × N=30 pulses
per run; expect ~minutes wall.

Output: numerics/fig6_alpha3_v3.h5 + plots/fig6{a,b}_v3.png
"""
...
DELTA_T_FRAC = 0.13                          # δt as fraction of T_m
T_M          = 2 * np.pi / OMEGA_M           # engine time units
DELTA_T      = DELTA_T_FRAC * T_M            # δt in engine time units
N_PULSES     = 30                            # Hasse Appendix D

# Choose Ω so that N·Ω_eff·δt = π/2 (π/2 train budget)
...
def main():
    out_h5 = os.path.join(SCRIPT_DIR, 'fig6_alpha3_v3.h5')

    print(f"Hasse-matched parameters (engine v{ss.CODE_VERSION}):")
    print(f"  T_m         = {T_M:.3f} engine_time_units")
    print(f"  δt          = {DELTA_T:.3f}  ({DELTA_T_FRAC*100:.0f}% of T_m)")
    print(f"  N_pulses    = {N_PULSES}")
    print(f"  Ω           = {OMEGA_R:.4f}  (Ω/ω_m = {OMEGA_R/OMEGA_M:.3f})")
    print(f"  Ω_eff       = {OMEGA_EFF:.4f}")
    print(f"  per-pulse θ = {OMEGA_EFF*DELTA_T:.4f} rad")
    print(f"  ω_m·δt      = {OMEGA_M*DELTA_T:.3f} rad  (Hasse ≈ 0.82)")
    print(f"  total θ_acc = {N_PULSES*OMEGA_EFF*DELTA_T:.4f} rad  (target π/2 = {np.pi/2:.4f})")
    print()
    ...
    with h5py.File(out_h5, 'w') as f:
        ...
        f.attrs['notes'] = 'Round 2: Hasse-matched (N=30, δt=0.13·T_m).'
    ...
    extras = (rf'N={N_PULSES}, $\delta t/T_m$={DELTA_T_FRAC:.2f}, '
              rf'$\omega_m\delta t$={OMEGA_M*DELTA_T:.2f}')
    heatmap(sigma_z_map,
            f'Hasse Fig 6a v3 — engine v{ss.CODE_VERSION}, Hasse-matched ({extras})',
            'fig6a_sigma_z_v3.png',
            cmap='RdBu_r', ylabel=r'$\langle\sigma_z\rangle$', vlim_floor=0.9)
    heatmap(delta_n_map,
            f'Hasse Fig 6b v3 — engine v{ss.CODE_VERSION}, Hasse-matched ({extras})',
            'fig6b_back_action_v3.png',
            cmap='PRGn', ylabel=r'$\delta\langle n\rangle$', vlim_floor=0.9)

    print(f"\n  σ_z map range:  [{sigma_z_map.min():+.3f}, {sigma_z_map.max():+.3f}]  (Hasse ±0.9)")
    print(f"  δ⟨n⟩ map range: [{delta_n_map.min():+.3f}, {delta_n_map.max():+.3f}]  (Hasse ±0.9)")

    # ϕ-amplitude vs ϑ₀-amplitude (the Round-1 issue: weak ϕ in v2)
    ...
    print(f"  Hasse Fig 6b expects these to be COMPARABLE.")
