...
| [`paper_draft.md`](./notes/paper_draft.md) (source) · [`paper_draft.html`](./notes/paper_draft.html) (rendered) | **⚠ Internal in-progress paper draft** — §II–V drafted, §I/VI/VII key-paragraphs, no figures, stub refs, provisional authors. **Not a preprint; not for external circulation** (scope: `publication_assessment.md` §7/§8). The `.md` is the source of truth; the `.html` is **generated by pandoc — do not hand-edit** (regen command in *Quick start* below). |
...
# Re-render the paper draft to HTML (after editing the .md source).
# Leading working-metadata comment (lines 1–19) stripped; a visible
# in-progress banner + a noindex meta injected. Partials are committed
# alongside (paper_draft.{head,banner,foot}.html) — fully reproducible,
# byte-identical output. Do not hand-edit paper_draft.html.
cd wp-wigner-tomography/notes
tail -n +20 paper_draft.md | pandoc -f markdown -t html5 -s --mathjax \
  --metadata title="WP-W — paper draft (internal, in progress)" \
  -H paper_draft.head.html -B paper_draft.banner.html \
  -A paper_draft.foot.html -o paper_draft.html
```
